...
## EJERCICIO 1 ##
n<-15
e<- seq(0,3, length=100)
plot(e,e^n*(e<1), type='l', main='Estadistico RV',ylab='RV',xlab='X(n)')
#potencia
h<-0.8189
theta<-seq(0.01, 4,length=200)
pot<- 1*(theta<h)+(h/theta)^n*(theta >h)+(1-(1/theta)^n)*(theta >1)
plot(theta,pot,type='l', main='Funcion Potencia',xlab='theta', ylab='pot')
#apartado f
pot1 <-(h/1.2)^n + (1-(1/1.2)^n);pot1#pot en el punto 1.2
n1<-n
while(pot1>0.8) {n1<-n1-1;h1<-0.05^(1/n1);
pot1<-(h1/1.2)^n1 + (1-(1/1.2)^n1)}
cat(c('tama~no de muestra necesario >=', n1+1),'\n')
###############################################################################
## EJERCICIO 2 ##
